...
export default function RecipeEdit({ recipe }) {
  ...
  function handleIngrdientChange(id, ingredient) {
  ...
  return (
    <div className="recipe-edit">
      ...
      <div className="recipe-edit__details-grid">
        ...
        <input type="text" min="1" name="servings" id="servings" className="recipe-edit__input" value={recipe.servings}  onChange={e => handleChange({ servings: parseInt(e.target.value) || '' })} />

        <label htmlFor="instructions" className="recipe-edit__label">Instructions</label>
        <textarea name="instuctions" id="instuctions" className="recipe-edit__input" value= {recipe.instructions} onChange={e => handleChange({ instructions: e.target.value })} />
      </div>
      <div>
        <br />
      ...
      <div className="recipe-edit__ingredient-grid">
        ...
        {recipe.ingredients.map(ingredient => (
          <RecipeIngredientEdit key = {ingredient.id} handleIngrdientChange ={ handleIngrdientChange} handleIngredientDelete={handleIngredientDelete} ingredient = {ingredient} />
        ))}
